...
def display_images(viewpoint="top", random_state=1, metadata=metadata):
    ...
    sample = metadata[metadata.viewpoint == viewpoint].sample(
        nrows * ncols, random_state=random_state
    )
    # plot in grid
...
plot_image_dims(
    metadata[metadata.viewpoint != "top"], axes[1], title="lateral images", ylabel=False
)
